...
''' 10/17/2017 - Introduced a drop down list containing cell phone email to text addresses to use to send images as a text message. I also
''' introduced another field to contain a link to a shorter message when using the text message option.
''' 10/19/2017 Fixed issue with RepeatEmailsInEmailPrompt not locking correctly.
'''</history>
'''</summary>
''' 
Public Class EPEForm1
    ...
    Public FirstLoop As Boolean = True
    ...
    Public Sub ImportDefaults(ByVal DefaultFileToUse As String)
        ...
        Textbox_TextMessageFile.Text = mydata.TextMessage
        ...
        Try
            ...
            End With
            RepeatEmailsInEmailPrompt = Convert.ToBoolean(mydata.RepeatEmailsInEmailPrompt)
        Catch ex As Exception
    ...
    Public Sub SaveAsXML(ByVal sender As Object, ByVal e As EventArgs) Handles Button_SaveAsDefault.Click
        ...
        MyInfoArraylist.Add("TextMessage" & "!" & Textbox_TextMessageFile.Text)
        ...
        MyInfoArraylist.Add("RepeatEmailsInEmailPrompt" & "!" & RepeatEmailsInEmailPrompt)
...
#Region "MainProgram"
    ...
    Public Sub SendParsedEmails(ByVal sender As Object, ByVal e As EventArgs) Handles BW_EmailerParcer.DoWork
        ...
        Dim oldfilename_arraylist As New ArrayList
        ...
        For Each fullstring In listing ' This loops parses the files and produces arrays for email() and filename()
            ...
            If FileReadyStatus = True Then
                ...
                If temp.Length < 2 Then 'indicates that there was no $ in the string. Will need to add the $ to proceed
                    ...
                    temp(0) = ""
                End If
                If temp.Length = 3 Then 'indicates that there was a $$. Replace the blank temp(1) with temp(2)
                    temp(1) = temp(2)
                End If
                'First copy image to the event folder truncating the email address(s)
                oldfilename = tempstring & "\" & fullstring.Name
                oldfilename_arraylist.Add(oldfilename)
                ...
                '                If EmailSetupForm.PromptForEmailDontSend_CheckBox.Checked Or EmailSetupForm.PromptForEmailAndSend_CheckBox.Checked And SENDEMAILFromEmailReadyFolderFLAGG = False Then
                If SENDEMAILFromEmailReadyFolderFLAGG = False Then
                    ...
                    Else
                        ...
                        End If
                    End If
                End If ' Check if Send Email From Folder is False
                ...
                If temp(0) <> "EmailLater" Then
                    ...
                    Else ' Saved Name feature is not used yet
                        ...
                        NameString = temp(1)
                    End If
                    ParseName = Split(NameString, "!") ' need to ensure that names match each email address.
                    ' NameString should be the same size as emailstring.
                    ...
                    If ParseEmail.Length > 1 Then
                        For jj = 0 To ParseEmail.Length - 1
                            ...
                            'CustNameArray(counter + jj) = ParseName(1).ToString ' Ideally, this value should not change
                            'TODO this is a BandAid Fix. The CustNameArray needs to be the same size as the emailarray.
                            'I need to do a better job handling the case for which no customer name is given.
                            'So, until this feature is fixed, the name will not be used.
                            CustNameArray(counter + jj) = ""
                        Next
                        counter += jj
                    Else
                        If counter >= 1 Then
                                        ...
                                        If (checkfilestring(kk) = tempB(1)) And (checkemailstring(kkk) = parseOriginalEmail(0)) Or PostEmailPromptYesNo = True Then
                                            ...
                                            If counter = NumOfEmailees Then
                                                Try
                                                    ...
                                                    End If
                                                    If SENDEMAILFromEmailReadyFolderFLAGG = True Then
                                                        For Each cc As String In oldfilename_arraylist
                                                            File.Delete(cc)
                                                        Next
                                                    End If
                                                Catch ex As Exception
        ...
        ' Changed on 10/19/2017 --- the state of RepeatEmailsInEmails will stay fixed
        ...
        emailarray = Nothing
        filenamearray = Nothing
        CustNameArray = Nothing
    End Sub
#End Region
    ...
    Public Sub SENDEMAIL(ByVal email As String, ByVal filenamestring As String, ByVal EventFolder As String, ByVal filenameshort As String, ByRef GOFLAGG As String, ByRef SENDEMAILFLAGG As Boolean, Optional ByVal EmailName As String = "")
        ...
        If CancelFlagg <> "Cancel" Then
            ...
            'If EmailOnlyRadio.Checked Then
    ...
    Public Sub CheckBox2_CheckedChanged_OLD(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UseEmailDirectory_Checkbox.CheckedChanged
        If UseEmailDirectory_Checkbox.CheckState = CheckState.Checked Then EmailFilenameList.Enabled = True Else EmailFilenameList.Enabled = False
        If UseEmailDirectory_Checkbox.CheckState = CheckState.Checked Then ignoreJPG.Enabled = True Else ignoreJPG.Enabled = False
        If UseEmailDirectory_Checkbox.CheckState = CheckState.Checked Then Button3_browse_email_filename.Enabled = True Else Button3_browse_email_filename.Enabled = False
        If UseEmailDirectory_Checkbox.CheckState = CheckState.Checked Then RichTextBox_EmailListDescription.Enabled = True Else RichTextBox_EmailListDescription.Enabled = False
    End Sub
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Public Sub Emailfilename_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UseEmailDirectory_Checkbox.CheckedChanged
        If UseEmailDirectory_Checkbox.CheckState = CheckState.Checked Then
            EmailFilenameList.Enabled = True
            ignoreJPG.Enabled = True
            Button3_browse_email_filename.Enabled = True
            RichTextBox_EmailListDescription.Enabled = True
    ...
    Public Sub MessageFileButton(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button_browse_message_file.Click
        OpenFileDialog1_message_location.ShowDialog()
        Textbox_TextMessageFile.Text = OpenFileDialog1_message_location.FileName
    End Sub
...
#End Region
    ...
    'Private Sub EmailAndFacebookRadio_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    AlbumComboBox.Enabled = True
    '    FacebookCaptionTextBox1.Enabled = True
    '    GoButton.Text = "Send to Email and Facebook"
    'End Sub

    'Private Sub FacebookOnlyRadio_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    AlbumComboBox.Enabled = True
    '    FacebookCaptionTextBox1.Enabled = True
            ...
            Else
                ...
                Try
                    With mail
                        ...
                        If IsNumeric(emailuse.Substring(0, 1)) Then
                            .Body = GetFileContents(Textbox_TextMessageFile.Text) ' have to write code to get contents of file
                        Else
    ...
    Public Function EmailPromptFunction(ByVal ImageAddress As String) As String
        ...
        ContinueOrCancel = ""
        MyEmailPrompt.TextBox1.Focus()
        While ContinueOrCancel <> "Cancel" And ContinueOrCancel <> "Continue"
            pause(100) ' needed to be able to recognize when the send button is hit.
            If RepeatEmailsInEmailPrompt = True And (ContinueOrCancel <> "Cancel" And ContinueOrCancel <> "Continue") Then ' Changes when send button is hit
                If FirstLoop = False Then ' Set to true when program starts and then reset to true after all emails have been sent
                    ...
                    'FirstLoop = False ' Sets to false after information is entered into the initial form
                End If
            End If
        End While
        FirstLoop = False
        ...
        If MyEmailPrompt.SaveForLaterCheckBox.Checked Then
            ...
            MyReturnString = "EmailLater"
            'Return "EmailLater"
        Else
            MyReturnString = emailstrings & "**" & emailnamesstrings
    ...
    Private Sub Button_browse_textmessage_Click(sender As System.Object, e As System.EventArgs) Handles Button_browse_textmessage.Click
        OpenFileDialog1_message_location.ShowDialog()
        TextBox_Messagefile.Text = OpenFileDialog1_message_location.FileName
    End Sub

    Private Sub TextBox_Messagefile_TextChanged(sender As System.Object, e As System.EventArgs) Handles TextBox_Messagefile.TextChanged

    End Sub

    Private Sub Label9_EmailListDescription_Click(sender As System.Object, e As System.EventArgs)

    End Sub

    Private Sub ignoreJPG_CheckedChanged(sender As System.Object, e As System.EventArgs) Handles ignoreJPG.CheckedChanged

    End Sub

    Private Sub RichTextBox1_TextChanged(sender As System.Object, e As System.EventArgs) Handles RichTextBox_EmailListDescription.TextChanged
