...
''' May 2012 - Removed requirement for licensing. 
'''          - Data is now stored in an 3 file.
'''</history>
'''</summary>
Public Class Form1
    ...
    Public Sub ImportDefaults(ByVal DefaultFileToUse As String)
        ...
        COMPDATA.CompanyEmail = mydata.CompanyEmail
        COMPDATA.CompanyName = mydata.CompanyName
        CustomerInfoLabel.Text = "Licensed to: " & COMPDATA.CompanyName
        ...
        CheckBox_ConfirmationEmail.CheckState = BooleanToCheckState(mydata.ConfimEmailBool)
        EmailCombineCheckBox.CheckState = BooleanToCheckState(mydata.CombineEmailsBool)
        Try
            With MyForm2
                .CompNameTextbox.Text = COMPDATA.CompanyName
                .CompEmailTextbox.Text = COMPDATA.CompanyEmail
...
Catch ex As Exception
    ...
    Public Sub SaveAsXML(ByVal sender As Object, ByVal e As EventArgs) Handles Button_SaveAsDefault.Click
        ...
        MyInfoArraylist.Add("UseMasterListBool" & "!" & MyForm2.UseMasterListCheckbox.CheckState)
        MyInfoArraylist.Add("ConfimEmailBool" & "!" & CheckBox_ConfirmationEmail.CheckState)
        MyInfoArraylist.Add("CombineEmailsBool" & "!" & EmailCombineCheckBox.CheckState)
        MyInfoArraylist.Add("UseSSLBool" & "!" & MyForm2.SSLcheckBox.CheckState)
        MyInfoArraylist.Add("CheckInternetConnectionBool" & "!" & MyForm2.Checkbox_CheckForIntConnectYesNo.CheckState)
        MyInfoArraylist.Add("PostEmailPromptBool" & "!" & MyForm2.PostEmailPrompt_CheckBox1.CheckState)
        MyInfoArraylist.Add("UseAdLabelBool" & "!" & MyForm2.IncludeAdCheckBox.CheckState)
    ...
    Public Sub SendParsedEmails(ByVal sender As Object, ByVal e As EventArgs) Handles BW_EmailerParcer.DoWork
        ...
        For Each fullstring In listing ' This loops parses the files and produces arrays for email() and filename()
            ...
            Else
                ...
                ' It is possible for filename to not have the $ if not using Darkroom (future/unforseen)
                ' in this case, temp(0)="imagefile.jpg" instead of temp(0)=email and temp(1) = file.
                If temp.Length = 1 Then
                    newfilename = EventFolder & "\" & temp(0)
                Else
            ...
            If PostEmailPromptYesNo = True And Not FacebookOnlyRadio.Checked Then
                ...
                End If
            End If
            If temp(0) = "" And temp.Length > 1 Then
                ParseEmail = Split(temp(1), "!")
            Else
    ...
    Public Sub SENDEMAIL(ByVal email As String, ByVal filenamestring As String, ByVal EventFolder As String, ByVal filenameshort As String, ByRef GOFLAGG As String, ByRef SENDEMAILFLAGG As Boolean)
        ...
        If CancelFlagg <> "Cancel" Then
            ...
            If EmailAndFacebookRadio.Checked Or EmailOnlyRadio.Checked Then
                ...
                If Len(COMPDATA.CompanyEmail) = 0 Then
...
#End Region
    ...
    Public Function MailHoodTest() As MAILERRORHOOD
        ...
        With mail
            ...
            .DeliveryNotificationOptions = DeliveryNotificationOptions.OnFailure
            .ReplyTo = New MailAddress(COMPDATA.CompanyEmail)
            '.To.Add(New MailAddress("epedummy@hoodandsons.com")) ' This email address is setup on my server and routes directly to the trash can. It is setup ONLY to test if a connection is good. (May need to try another way later)
