                ...
                Else
                    ...
                    Else
                        ...
                        If FacebookOnlyCheckBox.CheckState = CheckState.Checked Then
                            InputString = "[" & filenameshort & "]" & " was successfully sent to FACEBOOK" & vbCrLf
                            facebooksentcounter += 1
                            Facebooksentlabel.Text = facebooksentcounter.ToString
                        Else
...
#End Region
    ...
    ''' <returns>A structured variable based on HoodFacebookInfo which hold the desired info: AccessToken, ID, AID, and AlbumID</returns>
    ...
    ''' webbrowser window and ask for login information. MyAPI (a global variable) is assinged a value at that time. When GetAPIandAlbumID is called,
    ...
    ''' a webbrowser to log in to facebook and allow permissions to EPE's Facebook App.
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    ...
    Public Function MailHood(ByVal emailuse As String, ByVal filenames As String) As Integer
        'SendEmailCode = seeSendEmail(0, emailuse, NullString, NullString, textBox_SubjectLine.Text, APPENDEDMESSAGE, FileStringToUse)
        Dim SmtpServer As New SmtpClient()
        SmtpServer.Credentials = New Net.NetworkCredential(MyForm2.textBox_Username.Text, MyForm2.textBox_Password.Text)
        SmtpServer.Port = CType(MyForm2.PortBox.Text, Integer)
        SmtpServer.Host = MyForm2.textBox_MailServer.Text
        ...
        addr = Split(emailuse, ",")
        Try
            mail.From = New MailAddress(COMPDATA.CompanyEmail, COMPDATA.CompanyName, System.Text.Encoding.UTF8)
            Dim i As Integer
            For i = 0 To addr.Length - 1
                mail.To.Add(addr(i))
            Next
            mail.Subject = textBox_SubjectLine.Text
            mail.Body = textBox_SubjectLine.Text ' have to write code to get contents of file
            Dim filestoemail() As String = Split(filenames, ",")
            For i = 0 To filestoemail.Length - 1
                mail.Attachments.Add(New Attachment(filestoemail(i)))
            Next
            mail.DeliveryNotificationOptions = DeliveryNotificationOptions.OnFailure
            mail.ReplyTo = New MailAddress(CompanyEmail)
