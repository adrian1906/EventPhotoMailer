...
Public Class Form1
    ...
    Public Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        pause(2000)
        SplashScreen1.Hide()
        Me.Text = " Event Photo Emailer - Version: " & My.Application.Info.Version.Major.ToString & "." & My.Application.Info.Version.Minor.ToString
...
#Region "MainProgram"
    Public Sub RunGo()
        ...
        ' RunGo() checks to see if thread is busy first. If so, NewImagesAreReady  is set to TRUE. This is based on the possibility
        ' that the RunGo() event was raised as images were being added while emailing.
        ' The BW_LetsGo.RunWorkerCompleted subroutine will check the flag to decide if to rerun the emailer.
    ...
    Public Function PostPhotoOntoFacebook(ByVal IMAGESTRING As String, ByVal FACEBOOKALBUM As String, ByVal FACEBOOKCAPTION As String) As Boolean
        ...
        For counter = 0 To imagenames.Length - 1
            ...
            If IsNothing(CustomerAPI) Then
                Try
                    Dim CustomerInfo As HoodFacebookInfo
                    CustomerInfo = GetAPI(MyForm2.FacebookLoginEmailTextbox.Text, MyForm2.FaceboolLoginPasswordTextbox.Text, checktimerOK, MyWebBrowser)
                    CustomerAPI = CustomerInfo.MYAPIinfo
                    ...
                    FACEBOOKERROR = False
                    Dim ErrorMessage As String = PublishPhotoHOOD(B, FACEBOOKCAPTION, CustomerInfo)
            ...
            Else
                Try
                    ...
                    FacebookUploadSuccessfull = True
                Catch ex As Exception
                    ' do nothing FacebookUploadSuccessfull is False by default
                End Try
            End If
        Next
...
#End Region
    ...
    Public Function GetAPI(ByVal email As String, ByVal password As String, ByRef CHECKTIMEROK As Boolean, ByVal MyWebBrowser As WebBrowser) As HoodFacebookInfo
        ...
        Next
        If Myaid = Nothing Then
            Myaid = MakeAlbum3(FacebookAlbumName.Text, MyWebBrowser)
            Dim A As Album = New Album(Myaid, myAPI.AccessToken)
            MyAlbumID = A.id
        End If
        ' Get list of Friends
    ...
    Public Function MakeAlbum3(ByVal AlbumName As String, ByVal MyWebBrowser As WebBrowser) As String
        MyWebBrowser.Navigate("http://www.facebook.com/albums/create.php")
        WebWait(MyWebBrowser)
        Dim theElementCollection As HtmlElementCollection = MyWebBrowser.Document.GetElementsByTagName("input")
        For Each curElement As HtmlElement In theElementCollection
            Dim controlName As String = curElement.GetAttribute("name").ToString
            If controlName = "Name of album:" Then
        ...
        Next
        WebWait(MyWebBrowser)
        'http://www.facebook.com/editalbum.php?id=601201034&aid=304777&add=1&created=1
        Dim AccessURI As String = MyWebBrowser.Url.ToString
        Dim MyURI() As String
    ...
    Public Sub WebWait(ByVal MWB As WebBrowser)
        Do While Not MWB.ReadyState = WebBrowserReadyState.Complete
            'MsgBox("Still Busy")
            pause(1000)
        Loop
    End Sub
