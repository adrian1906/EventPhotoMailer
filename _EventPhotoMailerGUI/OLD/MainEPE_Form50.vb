...
Public Class Form1
    ...
    'Kenneth D. Photography ken@kennydphotography.com: IVzdxaMg2ZeIAAcytV8yyPZIe1OLnbZjCYs7kHTPgRIlxam4Wi37f5dzwcy0sv7Ns+fUpWYMnUojMvmKyZoYVw==
    'Kenneth D. Photography (new) Jg/D7lxVW4rtIsJsZIsOUdEkkx3HEihvAhuLSF6KRXDEpTBTVagApUTS/Z3hGg5fWiwL7zX7QalGWlCeukHfeQ=='
    'Myrle Metcalf $$$ <myrle@thephotostore.com> F2YZMbu5vZiK2Tw0voAE86kfKuKvWvxSvicCnzXHN2Yjsvs0xuQYMC0e3TGVBNNQ
    ...
    'MRU Testing  ox3pG0uf9S4MyXL2+PmGh9f+hcovGJ8DmLOM2+nATd7FBcnvd0yybP/dmyyK6j11KpMVLB6AWmtSKodx0Z69Ug==
    'My Easy Photo: 
    ...
    Dim NewImageAreReady As Boolean = False ' Used in autosend mode to detect that new images have been added while
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If BuildType = True Then
            ...
            ClientEmailAddressLabel.Visible = True
            ClientEmailAddressTextBox.Visible = True
        Else
            ClientNameTextBox.Visible = False
            ClientNameLabel.Visible = False
            ClientEmailAddressLabel.Visible = False
            ClientEmailAddressTextBox.Visible = False
        End If
        ...
        If File.Exists(DefaultFile) Then
            ...
            Try
                FacebookTextBox.Text = MyDecryption(d.ReadLine())
                If FacebookTextBox.Text = "InvalidPasscode$$$Corrupt" Then
                    FacebookTextBox.Text = "Enter Facebook Address"
                End If
            Catch ex As Exception
                FacebookTextBox.Text = "Enter Facebook Address"
            End Try
    ...
    Public Sub RunGo()
        ...
        'If files are created too fast, each file will generate a new instance and heads will bump when 
        'writing to the status window. I added code to check if the tread was already created. If so, then no new thread
        'would be created. It first attempts to check the status of the thread using IsAlive. A try-catch loop is used for
        'for the case that GOThread has not been initialized.  I suspect that a problem could arise if new files are being
        'created during the send process. The triggered events may be blocked until the thread terminates. I suspect that this
        'is a minor issue and would correct itself when new files enter the folder. Only the last set of files to enter the folder
        'may risk not being sent. Will need to check folder before closing out.
        Try
            If GOThread.IsAlive() Then
                NewImageAreReady = True ' used to trigger a re-run at the end of the current run
                'do nothing
            Else
        ...
        'MsgBox("Counts =" & Process.GetCurrentProcess().Threads.Count)
        'If Process.GetCurrentProcess().Threads.Count <= 1 Then
        'GOThread = New Thread(AddressOf LetsGO)
        'GOThread.Start()
        'End If
        'ListProcesses()
    ...
    Public Function CheckEmailConnection() As MAILERRORHOOD
        ...
        If Len(PortBox.Text) <> 0 Then
            ...
            Display("Checking for SMTP (This may take a few seconds...)")
        ...
        Else
            ...
            While d4.Peek() > -1
                ...
                If checkemail = False Then
                    ...
                    Display(email.ToCharArray & ErrorString)
                Else
    ...
    Private Sub SENDEMAIL(ByVal email As String, ByVal filenamestring As String, ByVal EventFolder As String, ByVal filenameshort As String, ByRef GOFLAGG As String, ByRef SENDEMAILFLAGG As Boolean)
        ...
        If CancelFlagg <> "Cancel" Then
            ...
            'check if client field has been filled in
            If BuildType = True Then
                If Len(ClientNameTextBox.Text) = 0 Then
                    Display("Missing Client Name")
                End If
            End If

            If BuildType = True Then
                If Len(ClientEmailAddressTextBox.Text) = 0 Then
                    Display("Missing Client Email")
                End If
            End If
            'connect to server
            ...
            If BuildType = True Then
                Dim ClientCompanyInfo As String = ClientNameTextBox.Text & " <" & ClientEmailAddressTextBox.Text & ">"
                APPENDEDMESSAGE = GetFileContents(textBox_messagefile.Text, UserCompanyName)
                'SaveTextToFile(APPENDEDMESSAGE, ProgDir2.Text & "\bin\e1t11906.dat") ' Each One Teach One 1906
                'tempfile = AppendText(textBox_messagefile.Text, vbCrLf & "Photography by: " & UserCompanyName)
                'AccessPOP3()
                SmtpErrorCode = seeSmtpConnect(0, textBox_MailServer.Text, ClientCompanyInfo, UserCompanyName)
            Else
                'AccessPOP3()
                SmtpErrorCode = seeSmtpConnect(0, textBox_MailServer.Text, UserCompanyName, NullString)
            End If
            ...
            Else
                SENDEMAILFLAGG = True
                Dim InputString As String = "[" & filenameshort & "]" & " was successfully sent to " & emailuse
                Display(InputString)
                My.Computer.FileSystem.WriteAllText(StatusFile, InputString & Chr(13), True)
    ...
    Private Sub Button_SaveAsDefault_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button_SaveAsDefault.Click
        ...
        s.WriteLine(MyEncryption(FacebookTextBox.Text))
        If MonitorFolderCheckBox.CheckState = CheckState.Checked Then
            s.WriteLine("1")
        Else
            s.WriteLine("0")
    ...
    Public Sub chkemailthread()
        Dim chkemail As New Thread(AddressOf CheckEmailConnection)
        chkemail.Start()
    End Sub

    Public Sub stmpthread()
        Dim runstmponthread As New Thread(AddressOf SubSMTPconnectHOOD)
        runstmponthread.Start()
    End Sub

    Public Sub SubSMTPconnectHOOD()
        'Dim SmtpErrorCode As Integer
        'SmtpErrorCodeGlobal = seeSmtpConnect(0, textBox_MailServer.Text, "<Test>", NullString)
        chkemailconn = True
    End Sub
...
exit_function:
    ...
    ' ************** Attempts at feedback while checking for SMTP connection ************
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CheckConnectionTimer.Tick
        checkinternetconnection()
    End Sub

    Private Sub SMTPdots_timer(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CheckSMTPtimerdots.Tick
        'MsgBox("OK Now What")
        StatusBox.Text = "ok, now what"
        If SmtpErrorCodeGlobal = 999 Then
            Display("Still Processing")
        End If
    End Sub

    Public Sub StatusIndicatorHoodonthread(ByVal chan As Integer, ByVal server As String, ByVal address As String, ByVal Replyto As String)
        ' The purpose of this subroutine is the create running dots as feedback
        ' as a check for SMTP was being made.
        ' So far, unsuccessful
        Dim runstatusdots As New Thread(DirectCast(Function() seeSmtpConnect(chan, server, address, Replyto), ThreadStart))
        runstatusdots.Start()
    End Sub
    ...
    Public Sub StatusIndicatorHood()
        ...
        While SmtpErrorCodeGlobal = 999 And timecheck < maxduration ' Timeout set to 10 seconds.
            ...
            pause(1000)
            StatusBox.Text = "Searching for SMTP server: " & timecheck & "/" & maxduration
        End While
        Control.CheckForIllegalCrossThreadCalls = True
    End Sub
    ...
    Private Sub CheckBox1_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CheckBox1.CheckedChanged
        If CheckBox1.CheckState = CheckState.Checked Then
            FacebookTextBox.Visible = True
        Else
            FacebookTextBox.Visible = False
        End If
    End Sub
