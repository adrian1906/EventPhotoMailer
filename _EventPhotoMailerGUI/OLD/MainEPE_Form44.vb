...
'9-5 Added the ability to change the FROM address. This version that is compiled is set by the BUILDTYPE variable.
'This sets the From to the company name provided (email address must be valid). The reply to address is still
'The company's. In addition, "Photography by COMPANY NAME <email.com>" is added to the bottom of each message file.
'A few notes:
'1.) the use of *****  (5 stars) will cause mailer program to ignore text. For example, if I add
'***** Photography by ABC Photo *****     This line of text will be ignored.
'2.) Its possible to minimize my security feature by producing a message file that has a lot of spaces at the end. 
'For which my appended text will not be seen and serve less of a deterrant.
Public Class Form1
