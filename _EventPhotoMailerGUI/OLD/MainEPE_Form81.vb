...
''' address is set to route all mail to the trash so no info is saved on my server. In cleaning up my site, I had deleted this email ''' address. This caused a Mail Not Deliverable Error. I re-created the email address and hopefully I will not forget what its for.
'''</history>
'''</summary>
Public Class Form1
...
#Region "MainProgram"
        ...
        If CancelFlagg <> "Cancel" Then
            ...
            Else ' Proceed forward
                ...
                If listing.Length > 0 Then
                    ...
                    End If
                Else
                    Display(StatusBox, "There are no images to send")
                End If
            End If
        End If
    ...
    ' ''' <summary>
    ' ''' CheckEmailConnection() checks if SMTP connection is available.
    ' ''' </summary>
    ' ''' <returns></returns>
    ' ''' <remarks>Note: Comcast,Verizon FIOS: 587 </remarks>
...
#End Region
    ...
    ''' <summary>
    ''' HOODFileWatcherJPG()
    ''' This subroutine was added to add intuition to the code. Before this, watchfolder was being used and I experienced
    ''' unexplained behaviour. For one, I kept getting threading errors. This event is triggered by a timer. At a 
    ''' specified interval, the folder is checked for new JPEG images. If at least one image exists, RunGO() is called.
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ...
    Public Function PostPhotoOnFacebookFGT(ByVal IMAGESTRING As String, ByVal FACEBOOKCAPTION As String) As Boolean
        ...
        For counter = 0 To imagenames.Length - 1
            ...
            Try
                ...
                Display(StatusBox, "Uploading image " & tmp(tmp.Length - 1) & " to Facebook.")
    ...
    '            Try
    '                Display(StatusBox, "Processing Facebook ...")
    '                FacebookCustomerInfo = GetAPIandAlbumID(checktimerOK, MyWebBrowser)
    '            Catch ex As Exception
    '                If FACEBOOKERRORONCE = True Then
    '                    MsgBox("Error: " & ex.Message & "  There was a problem uploading to Facebook. Please check your internet connection and/or Facebook settings.")
    '                    FACEBOOKERRORONCE = False
    ...
    ''' <summary>
    ''' SENDEMAIL is the main program to send emails. It is called both by Parseemail and Sendemailfromdirectory
    ''' </summary>
            ...
            If EmailAndFacebook.CheckState = CheckState.Checked Or EmailOnlyCheckbox.CheckState = CheckState.Checked Then
                ...
                If BuildType = True Then
                    Try
                        ...
                        'SendEmailCode = seeSendEmail(0, emailuse, NullString, NullString, textBox_SubjectLine.Text, APPENDEDMESSAGE, FileStringToUse)
                    Catch ex As Exception
                ...
                Else
                    Try
                        ...
                        'SendEmailCode = seeSendEmail(0, emailuse, NullString, NullString, textBox_SubjectLine.Text, "@" & textBox_messagefile.Text, FileStringToUse)
                    Catch ex As Exception
                        SendEmailCode = MailHood(emailuse, filenamestring)
                ...
                If CheckBox_ConfirmationEmail.CheckState = CheckState.Checked Then
                    Try
                        ...
                        'SendEmailCode = seeSendEmail(0, CompanyEmail, NullString, NullString, "Confirmation: " & textBox_SubjectLine.Text, "@" & textBox_messagefile.Text, FileStringToUse)
                    Catch ex As Exception
    ...
    End Sub
    ' ''' <summary>
    ' ''' ShowErrorSendingFile() This is used to display the error in the Statusbox, in the status.txt file and also compiled into an errors.txt file
    ' ''' the code corresponds to a database of codes in the see4VB library.
    ' ''' </summary>
    ' ''' <param name="ErrCode">Integer</param>
...
#End Region
    ...
    ''' <summary>
    ''' This program checks to see if files exist that start with $. If so, it is an indication that no email was attached.
        ...
        If CHECKCONNECTIONYESNO = True Or InitialConnectionCheck = True Then
            Try
                If My.Computer.Network.IsAvailable Then
                    Dim pingtmp As Boolean = My.Computer.Network.Ping(SiteToPing)
...
#End Region
    ...
    Public Function GetAPIandAlbumID(ByRef CHECKTIMEROK As Boolean, ByVal MyWebBrowser As WebBrowser) As HoodFacebookInfo
        ...
        'Disable checkinternetconnection - assume connection
        Dim INTCHECK As String
        '''''INTCHECK = checkinternetconnection()
        'TODO Currently, a temparary fix assumes that an internet connection exists. This needs a more robust solution.

        INTCHECK = "1" ' Temp. fix. Does not check internet connection first.
    ...
    Public Function MailHood(ByVal emailuse As String, ByVal filenames As String) As MAILERRORHOOD
        ...
        Catch ex As Exception
            If tryagain <= MaxTries Then
                tryagain += 1
                Display(StatusBox, "Retrying the SMTP connection. Try: " & tryagain)
