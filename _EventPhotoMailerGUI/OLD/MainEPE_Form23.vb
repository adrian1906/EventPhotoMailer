...
Public Class Form1
    ...
    Dim CompanyEmail As String = "<kirksphotos1@mac.com>" ' Email program needs <>
    Dim UserCompanyName As String = "Kirk Queen Photos" & " " & CompanyEmail
        ...
        If GoFlagg = True Then 'Global variable use to ensure this call to GoButton is only done once.
                ...
                Else
                    ...
                    While d4.Peek() > -1
                        ...
                        If testspace_email.Length < 2 Then
                            SENDEMAIL(email, filename, EventFolder, filenameshortDirectory)
                        Else
            ...
            Else ' Case for which filenames are parsed to get email addresses and filenames
                ...
                For Each fullstring In listing
                    ...
                    End If
                Next
                '                For counter = 0 To emailarray.Length - 1
                '                    MsgBox("Email: " & emailarray(counter) & " filenamearray: " & filenamearray(counter) & " filenameshort: " & filenameshort(counter))
                '                Next
                'MsgBox("Stop")
                'emailarrayCOPY = emailarray
                'filenamearrayCOPY = filenamearray
                Array.Sort(emailarray, filenamearray) ' Sorts both email and filename.. Key is on email, filename just follows suit.
                For counter = 0 To filenamearray.Length - 1
                    Dim temp4() As String = Split(filenamearray(counter), "\")
                    filenameshort(counter) = temp4(temp4.Length - 1)
                    'MsgBox("Email: " & emailarray(counter) & " filenamearray: " & filenamearray(counter) & " filenameshort: " & filenameshort(counter))
                Next
                ...
                '                    MsgBox("Email: " & emailstring(counter) & vbCrLf & _
                '                           "filenamestring: " & filenamestring(counter) & vbCrLf & _
                '                           "filenameshortstring: " & filenameshortstring(counter))
                '                Next
                'MsgBox("Stop")
                For counter = 0 To NumOfEmailees
                    ...
                    Else
                        ...
                        SENDEMAIL(emailstring(counter), filenamestring(counter), EventFolder, filenameshortstring(counter))
                    End If
    ...
    Private Sub SENDEMAIL(ByVal email As String, ByVal filenamestring As String, ByVal EventFolder As String, ByVal filenameshort As String)
