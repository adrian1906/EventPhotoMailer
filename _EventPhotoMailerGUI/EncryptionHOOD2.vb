...
#Region "  Utils"
    ...
    Friend Class Utils
        ...
        ''' </summary>
        Friend Shared Function GetConfigString(ByVal key As String, Optional ByVal isRequired As Boolean = True) As String
            'Dim s As String = CType(ConfigurationSettings.AppSettings.Get(key), String)
            Dim s As String = ConfigurationManager.ConnectionStrings(key).ConnectionString
            'Dim s As String = System.Configuration.ConfigurationSettings.AppSettings.Get(key).ToString
            If s = Nothing Then
                If isRequired Then
                    ...
                    MsgBox("key <" & key & "> is missing from .config file")
                Else
                    Return ""
                End If
            Else
